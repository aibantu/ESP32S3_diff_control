...
//PID
// 参数分别是P I D ramp limit
PIDController vel_loop_pid{2,0,0,100000,voltage_power_supply/2};
PIDController angle_loop_pid{2,0,0,100000,100};
...
void DFOC_M0_SET_ANGLE_PID(float P,float I,float D,float ramp)   //M0角度环PID设置
{
  ...
  angle_loop_pid.I=I;
  angle_loop_pid.D=D;
  angle_loop_pid.output_ramp=ramp;
}
...
{
   return vel_loop_pid(error);
   
}
//M0角度PID接口
float DFOC_M0_ANGLE_PID(float error)
{
  return angle_loop_pid(error);
}
...
void DFOC_Vbus(float power_supply)
{
  ...
  //PID 加载
  vel_loop_pid = {2, 0, 0, 100000, voltage_power_supply/2};
 }
